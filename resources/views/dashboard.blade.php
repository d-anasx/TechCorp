        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            ...
            <div class="bg-gray-800 p-6 rounded-2xl border border-gray-700">
                @if (auth()->user()->role->status === 'employee')
                    ...
                    <p class="text-4xl font-black text-white">{{ auth()->user()->tokens }} <span class="text-sm text-yellow-500 italic">TK</span></p>
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-3xl p-8 shadow-2xl">
             @if (auth()->user()->role->status === 'employee')
                ...
                <div class="flex items-center justify-between p-4 bg-gray-900 rounded-xl">
                    ...
                    <span class="px-3 py-1 bg-amber-500/10 text-amber-500 rounded-lg text-[10px] font-black uppercase">En attente</span>
                </div>
            @elseif (auth()->user()->role->status === 'manager')
            ...
            @else
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <button class="p-4 bg-gray-900 rounded-2xl hover:bg-gray-700 transition border border-gray-700 font-bold">LOGS</button>
                    <button class="p-4 bg-gray-900 rounded-2xl hover:bg-gray-700 transition border border-gray-700 font-bold">BACKUPS</button>
                    <button class="p-4 bg-gray-900 rounded-2xl hover:bg-gray-700 transition border border-gray-700 font-bold">MAINTENANCE</button>
                    <button class="p-4 bg-gray-900 rounded-2xl hover:bg-gray-700 transition border border-gray-700 font-bold">API</button>
                </div>
            @endif
        </div>
    </main>
